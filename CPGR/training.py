...
if __name__ == "__main__":
    ...
    """Hyperparameters."""
    DATASET, radius, ngram, dim, layer_gnn, window, layer_cnn, layer_output, lr, lr_decay, decay_interval, weight_decay, iteration = 'GPCR',1,3,128,3,11,3,3,1e-3,0.5,10,1e-7,31
    n_val=5
    ...
    AUCs = ('n_val\tEpoch\tTime(sec)\tLoss_train\tAUC_dev\tPRC_dev\t'
            'AUC_test\tPRC_test')
    ...
    dir_input = ('../input/radius' + str(radius) + '_ngram' + str(ngram) + '/')
    ...
    dataset_ = shuffle_dataset(dataset_, 0)
    T_v, Y_v, S_v=torch.tensor([]), torch.tensor([]), torch.tensor([])
    for i in range(n_val):
        file_model = '../output/model/' + str(i) + '_' +str(radius) + '_' + setting
        dataset_dev, dataset_train = val(dataset_, n_val, i)
        torch.manual_seed(k+i)
        ...
        for epoch in range(1, iteration):
            ...
            time = end - start
            AUCs = [i, epoch, time, loss_train, AUC_dev, PRC_dev, AUC_test, PRC_test]
