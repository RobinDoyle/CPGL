...
class CompoundProteinInteractionPrediction(nn.Module):
    ...
    def __call__(self, fingerprints, adjacency, words, correct_interaction, train=True):
        predicted_interaction, weights = self.forward(fingerprints, adjacency, words)
        if train:
...
class Trainer(object):
    ...
    def train(self, dataset):
        ...
        loss_total = 0
        for data in dataset:
            fingerprints, adjacency, sequence, correct_interaction = data
            words = torch.tensor(get_protein_embedding(word_model,seq_to_kmers(sequence)), dtype = torch.float32).to(device)
            loss = self.model(fingerprints, adjacency, words, correct_interaction)
    ...
    def test(self, dataset):
        ...
        for data in dataset:
            ...
            (correct_labels, predicted_labels,
             predicted_scores, _) = self.model(fingerprints, adjacency, words, correct_interaction, train=False)
...
if __name__ == "__main__":
    ...
    """Hyperparameters."""
    DATASET, radius, ngram, dim, layer_gnn, window, layer_cnn, layer_output, lr, lr_decay, decay_interval, weight_decay, iteration = 'kinase',0,3,16,3,11,3,3,1e-3,0.5,10,1e-5,11
    n_val=5
    ...
    with open('../input/kinase.txt', 'r') as f:
        data_list = f.read().strip().split('\n')
    """Exclude data contains '.' in the SMILES format."""
    data_list = [d for d in data_list if '.' not in d.strip().split()[0]]
    N = len(data_list)
    word_model = Word2Vec.load("../input/word2vec_100.model")
    ...
    dataset_ = dataset[:91552]
    dataset_test = dataset[91552:]
    dataset_ = shuffle_dataset(dataset_, 1234)
    file_model = '../output/model/'+ DATASET + '/' +str(radius) + '_' + setting
    dataset_dev, dataset_train = val(dataset_, n_val, 0)
    ...
    for epoch in range(1, iteration):
        ...
        AUC_dev, PRC_dev, _, _, _ = tester.test(dataset_dev)
        AUC_test, PRC_test, _, _, _ = tester.test(dataset_test)
        if temp > AUC_dev:
            trainer.optimizer.param_groups[0]['lr'] *= lr_decay
        end = timeit.default_timer()
        time = end - start
        AUCs = [radius, epoch, time, loss_train, AUC_dev, PRC_dev, AUC_test, PRC_test]
